...
export class FanPage implements OnInit {
  ...
  select(fan: device) {
    this.myForm.controls['id'].setValue(fan.id);
    this.myForm.controls['type'].setValue(fan.type);
    this.myForm.controls['status'].setValue(fan.status);
    this.myForm.controls['info'].setValue(fan.info);
  }
  ngOnInit(): void {//实现页面的初始化
    this.init();
    this.device$ = <Observable<device>>this.httpClient.get(this.baseUrl + 'device/fan/0');
  }
  init() {
  ...
  search() {
    if (this.id.value) {
      this.device$ = <Observable<device>>this.httpClient.get(this.baseUrl + 'device/fan/' + this.id.value);
      this.init();
    }
    else {
      this.device$ = <Observable<device>>this.httpClient.get(this.baseUrl + 'device/fan/0');
  ...
  delete() {
    ...
    if (this.currentUser.id == '') {
      ...
      this.httpClient.delete(this.baseUrl + 'device/fan/' +
