...
export class LightPage implements OnInit {
  ...
  select(led: device) {
    this.myForm.controls['id'].setValue(led.id);
    this.myForm.controls['type'].setValue(led.type);
    this.myForm.controls['status'].setValue(led.status);
    this.myForm.controls['info'].setValue(led.info);
  }
  ngOnInit(): void {//实现页面的初始化
    this.init();
    this.device$ = <Observable<device>>this.httpClient.get(this.baseUrl + 'device/led/0');
  }
  init() {
  ...
  search() {
    if (this.id.value) {
      this.device$ = <Observable<device>>this.httpClient.get(this.baseUrl + 'device/led/' + this.id.value);
      this.init();
    }
    else {
      this.device$ = <Observable<device>>this.httpClient.get(this.baseUrl + 'device/led/0');
  ...
  delete() {
    ...
    if (this.currentUser.id == '') {
      ...
      this.httpClient.delete(this.baseUrl + 'device/led/' +
