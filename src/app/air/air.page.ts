...
export class AirPage implements OnInit {
  ...
  select(air: device) {
    this.myForm.controls['id'].setValue(air.id);
    this.myForm.controls['type'].setValue(air.type);
    this.myForm.controls['status'].setValue(air.status);
    this.myForm.controls['info'].setValue(air.info);
  }
  ngOnInit(): void {//实现页面的初始化
    this.init();
    this.device$ = <Observable<device>>this.httpClient.get(this.baseUrl + 'device/air-conditioner/0');
  }
  init() {
  ...
  search() {
    if (this.id.value) {
      this.device$ = <Observable<device>>this.httpClient.get(this.baseUrl + 'device/air-conditioner/' + this.id.value);
      this.init();
    }
    else {
      this.device$ = <Observable<device>>this.httpClient.get(this.baseUrl + 'device/air-conditioner/0');
  ...
  delete() {
    ...
    if (this.currentUser.id == '') {
      ...
      this.httpClient.delete(this.baseUrl + 'device/air-conditioner/' +
